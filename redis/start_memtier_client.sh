...
MEMTIER_CMD=$PWD/memtier_benchmark

last_redis=0
last_memtier=0

redis_end_core=$(($1-1))
memtier_end_core=$(($redis_end_core+24))

#redis object data size,default 32, bytedance ask for 128, we may need higher for more mem throuput on redis server
redis_object_data_size=$2
...
start_redis-server(){
    ...
    mpstat -P `echo $(seq 0 1 $redis_end_core)|sed -e 's/ /,/g'` 30 1 > logs/cpuutil/test${1}-cpu.log&
...
start_memtier_benchmark(){
    ...
    for i in $(seq 24 1 $memtier_end_core); do
      ...
      numactl -C $i,$memtier_ht $MEMTIER_CMD -s 10.0.0.100 -p ${port} --pipeline=30 -c 2 -t 2 -d $redis_object_data_size --key-maximum=42949 --key-pattern=G:G --key-stddev=1177 --ratio=1:1 --distinct-client-seed --random-data --test-time=60 --run-count=1 --hide-histogram &>logs/${1}/redis-core${i}.log &
      last_memtier=$!
      echo $last_memtier
      port=$((${port}+1))
    done
