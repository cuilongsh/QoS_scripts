...
do
    ...
    iptables -t nat -A DOCKER ! -i docker0 -p tcp --dport 9000 -m state --state NEW -m statistic --mode nth --every $load_balence --packet 0 -j DNAT --to-destination 172.18.0.$docker_ip:80
